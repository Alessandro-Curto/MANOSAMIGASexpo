// Ruta: src/main/java/com/manosamigas/app/service/EventoService.java
...
public class EventoService {
    ...
    public EventoDto crearEvento(EventoDto dto) {
        // --- INICIO DEL CAMBIO: VALIDACIÓN DE DUPLICADOS ---
        if (existeEventoIdentico(dto)) {
            throw new DuplicateKeyException("Ya existe un evento con los mismos detalles.");
        }
        // --- FIN DEL CAMBIO ---
    ...
    // --- INICIO DEL CÓDIGO AÑADIDO: MÉTODO DE VALIDACIÓN ---
    private boolean existeEventoIdentico(EventoDto dto) {
        String sql = "SELECT COUNT(*) FROM Eventos WHERE nombre = ? AND descripcion = ? AND fecha_inicio = ? AND fecha_fin = ? AND ubicacion = ? AND cupo_maximo = ? AND id_interes_requerido = ?";
